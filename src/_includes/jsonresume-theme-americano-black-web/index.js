...
function groupByPosition(data, key) {
	...
	for (const item of data) {
		...
		grouped[item[key]].startDate = moment
			.min(moment(item.startDate), moment(grouped[item[key]].startDate))
			.format("YYYY-MM");
		if (item.endDate) {
			grouped[item[key]].endDate = moment
				.max(moment(item.endDate), moment(grouped[item[key]].endDate))
				.format("YYYY-MM");
		}
		grouped[item[key]].duration = getDurationString(
			grouped[item[key]].startDate,
			grouped[item[key]]?.endDate,
		);
	}
